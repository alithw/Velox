...
public:
    ...
    inline void WriteBit(int bit) {
        if (bit) bit_acc |= (1ULL << bit_count);
        bit_count++;
        if (bit_count == 8) { buffer.push_back(bit_acc & 0xFF); bit_acc = 0; bit_count = 0; }
    }
    
    inline void Write(uint32_t value, int num_bits) {
        for(int i=0; i<num_bits; i++) WriteBit((value >> i) & 1);
    }
    
    inline void Flush() { if (bit_count > 0) buffer.push_back(bit_acc & 0xFF); }
    const std::vector<uint8_t>& GetData() const { return buffer; }
    
    inline int ReadBit() {
        if (bit_count == 0) {
            if (read_pos >= buffer.size()) return 0;
    ...
    inline uint32_t Read(int num_bits) {
        uint32_t val = 0;
        for(int i=0; i<num_bits; i++) if (ReadBit()) val |= (1 << i);
        return val;
    }
    
    inline int32_t ReadSigned(int num_bits) {
        uint32_t val = Read(num_bits);
        if (val & (1 << (num_bits - 1))) return (int32_t)(val - (1 << num_bits));
        return (int32_t)val;
    }
};
...
private:
    ...
    static void ComputeLPC(const std::vector<int32_t>& data, int order, std::vector<int>& q_coeffs, int& q_shift) {
        ...
        for (int i = 0; i <= order; ++i) {
            double sum = 0;
            for (size_t j = i; j < data.size(); ++j) sum += windowed[j] * windowed[j - i];
            autocorr[i] = sum;
        }
    ...
    static void WriteZRL(BitStream& bs, uint32_t val, int k, int& zero_run) {
        ...
        if (q < 32) {
            for(uint32_t x=0; x<q; x++) bs.WriteBit(1);
            bs.WriteBit(0);
            if(k>0) bs.Write(r, k);
        } else {
            for(uint32_t x=0; x<32; x++) bs.WriteBit(1);
            bs.WriteBit(0);
            bs.Write(val, 32);
        }
    }
    ...
    static void EncodeBlock(const std::vector<int32_t>& data, BitStream& bs, uint32_t global_context) {
        ...
        if (lpc_coeffs.size() != order) { lpc_coeffs.assign(order, 0); lpc_shift = 0; }
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            for(int j=0; j<order; j++) {
                if (i > j) sumLPC += (int64_t)lpc_coeffs[j] * data[i-1-j];
            }
    ...
    static std::vector<int32_t> DecodeBlock(BitStream& bs, size_t count, uint32_t global_context) {
        ...
        std::vector<int> lpc_coeffs;
        for(int i=0; i<order; i++) lpc_coeffs.push_back(bs.ReadSigned(16));
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            int64_t sumLPC = 0;
            for(int j=0; j<order; j++) {
                if (i > j) sumLPC += (int64_t)lpc_coeffs[j] * output[i-1-j];
            }
            int32_t predLPC = (int32_t)(sumLPC >> lpc_shift);
    ...
    static uint32_t AnalyzeGlobalContext(const std::vector<int16_t>& pcm) {
        ...
        for(size_t i=0; i<pcm.size(); i+=10) sum += std::abs(pcm[i]);
        if (pcm.size() == 0) return 256;
