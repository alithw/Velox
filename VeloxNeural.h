...
public:
    ...
    inline int32_t Predict() {
        ...
        // Slow Brain
        for(int i=0; i<LONG_ORDER; i++) sum_long += (int64_t)history[i] * weights_long[i];
        
        // Fast Brain
        for(int i=0; i<SHORT_ORDER; i++) sum_short += (int64_t)history[i] * weights_short[i];
    ...
    inline void Update(int32_t actual, int32_t predicted) {
        ...
        if (sign == 0) {
            ...
            for(int i=LONG_ORDER-1; i>0; i--) history[i] = history[i-1];
            history[0] = actual;
            return;
        }
        ...
        for(int i=0; i<SHORT_ORDER; i++) {
            ...
            if (sign == isign) weights_short[i] += 2; // Tăng nhanh
            else if (isign != 0) weights_short[i] -= 2;
        }

        // Cập nhật Slow Brain (Conservative)
        for(int i=0; i<LONG_ORDER; i++) {
            int isign = (history[i] > 0) ? 1 : ((history[i] < 0) ? -1 : 0);
            if (sign == isign) weights_long[i] += 1;
            else if (isign != 0) weights_long[i] -= 1;
            
            // Leakage control để tránh overflow
            if (weights_long[i] > 65536) weights_long[i] = 65536;
            if (weights_long[i] < -65536) weights_long[i] = -65536;
        }
...
public:
    ...
    void SetInitialState(uint32_t init_val) { if(init_val > 0) mean_energy = init_val; }

    inline int GetK() const {
        if (mean_energy == 0) return 0;
